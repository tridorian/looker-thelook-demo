    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `tridorian-wildan-sandbox-dev.ecomm.order_items` as oi
                  JOIN `tridorian-wildan-sandbox-dev.ecomm.inventory_items` as inv ON inv.id = oi.inventory_item_id
      ...
      JOIN (SELECT user_id as user_id1
            ...
            FROM `tridorian-wildan-sandbox-dev.ecomm.events`
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `tridorian-wildan-sandbox-dev.ecomm.kmeans_model5`,
        (SELECT * FROM
